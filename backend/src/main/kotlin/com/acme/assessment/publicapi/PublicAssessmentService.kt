...
class PublicAssessmentService(
    ...
    private val candidateSessions: CandidateSessionService,
    private val objectMapper: ObjectMapper,
    private val clock: Clock,
) {
    fun cookieName(): String = candidateSessions.cookieName()

    @Transactional
    fun open(rawToken: String): PublicAssessmentResponse {
    ...
    fun getDraft(rawToken: String, sessionId: String?): PublicAssessmentResponse {
        val task = candidateSessions.requireSession(rawToken, sessionId)
        return toResponse(task)
    }

    @Transactional
    fun saveDraft(rawToken: String, sessionId: String?, request: SaveDraftRequest): SaveDraftResponse {
        val task = candidateSessions.requireSession(rawToken, sessionId)
    ...
    @Transactional
    fun submit(rawToken: String, sessionId: String?, request: SubmitAssessmentRequest): SubmitAssessmentResponse {
        val task = candidateSessions.requireSession(rawToken, sessionId)
        ...
        if (version.versionStatus != TemplateVersionStatus.PUBLISHED) {
            throw ConflictException("TEMPLATE_VERSION_UNAVAILABLE", "测评模板版本不可用")
        }
        ...
        assignmentRepository.findAllByTaskId(requireNotNull(task.id)).forEach { assignment ->
            if (assignment.status == AssignmentStatus.WAITING_CANDIDATE) {
    ...
    private fun toResponse(task: AssessmentTask): PublicAssessmentResponse {
        ...
        return PublicAssessmentResponse(
            ...
            candidatePhoneRequired = task.candidatePhone != null,
    ...
    private fun validateRequiredAnswers(task: AssessmentTask, schema: JsonNode) {
        ...
        val missing = questions.filter { it.path("required").asBoolean(false) }
            .mapNotNull { question ->
