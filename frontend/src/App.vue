...
const recordFilters = ref({ keyword: '', positionId: '', conclusion: '', reviewedFrom: '', reviewedTo: '' })
...
async function loadRecords() {
  ...
  try {
    ...
    if (currentFilters.reviewedFrom) query.set('reviewedFrom', new Date(currentFilters.reviewedFrom).toISOString())
    if (currentFilters.reviewedTo) query.set('reviewedTo', new Date(currentFilters.reviewedTo).toISOString())
...
function conclusionClass(value) {
  return { PASS: 'conclusion-pass', REJECTED: 'conclusion-rejected', RESERVED: 'conclusion-reserved' }[value] || ''
}
    ...
    <section class="content">
      ...
      <template v-else-if="view === 'records'">
        ...
        <section class="record-toolbar">
          ...
          <select v-model="recordFilters.conclusion"><option value="">全部结论</option><option value="PASS">通过</option><option value="REJECTED">不通过</option><option value="RESERVED">保留</option></select>
          <label>评估开始<input v-model="recordFilters.reviewedFrom" type="datetime-local" /></label>
          <label>评估结束<input v-model="recordFilters.reviewedTo" type="datetime-local" /></label>
        ...
        <section class="table-section"><div class="section-heading"><div><h2>候选人历史测评</h2><span>{{ recordTotal }} 条记录</span></div><span v-if="recordLoading" class="loading">正在加载...</span></div>
          <div class="table-scroll"><table><thead><tr><th>候选人</th><th>岗位</th><th>任务编号</th><th>模板版本</th><th>评估结论</th><th>评估时间</th><th>状态</th><th></th></tr></thead><tbody><tr v-for="record in records" :key="record.id"><td><strong>{{ record.candidateName }}</strong><small>{{ record.candidatePhone || record.candidateEmail || '--' }}</small></td><td>{{ record.positionName }}</td><td>{{ record.taskNo }}</td><td>v{{ record.templateVersionNo }}</td><td><div class="record-conclusions"><span v-for="(conclusion, index) in record.conclusions" :key="`${conclusion}-${index}`" class="conclusion-pill" :class="conclusionClass(conclusion)">{{ conclusionText(conclusion) }}</span><span v-if="!record.conclusions.length" class="muted">未提交</span></div></td><td>{{ formatDate(record.reviewedAt) }}</td><td><span class="status-pill" :class="statusClass(record.status)">{{ statusText(record.status) }}</span></td><td><button class="detail-button" @click="openRecord(record.id)">查看记录 →</button></td></tr><tr v-if="!records.length"><td colspan="8" class="empty">暂无符合条件的测评记录</td></tr></tbody></table></div>
